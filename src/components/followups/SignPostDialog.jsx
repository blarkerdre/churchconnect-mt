...
import { Loader2, Users, Home, Sparkles, MapPin, UserCircle2, AlertTriangle, X } from "lucide-react";
...
class DialogErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("[SignPost] render crash", error, info);
  }
  render() {
        ...
        <div className="space-y-3 py-4">
          <div className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3">
            ...
            <div className="text-xs text-destructive">
              <p className="font-semibold mb-1">Something went wrong opening this dialog.</p>
...
export default function SignPostDialog({ open, onOpenChange, followup, member, onCreated, defaultType = "unit_leader" }) {
  ...
  // Toast guards (fire once per relevant change) — gated by type to avoid loops
  const noUnitLeadersToastRef = useRef(null);
  useEffect(() => {
    if (!open || type !== "unit_leader" || !unitName) return;
  ...
  }, [open, type, unitName, unitLeaders.length]);
  ...
  }, [open, type, centres.length, member?.id, !!suggestedCentre]);

  const noLeaderLinkedToastRef = useRef(null);
  useEffect(() => {
    if (!open || type !== "home_cell_leader") return;
  ...
  }, [open, type, selectedCentre?.id, centreLeader?.linked]);
