...
export default function WoFBIPersistentQRDialog({ open, onOpenChange }) {
  const { currentTenant, tenantSlug } = useTenant();
  const qrRef = useRef();

  const url = tenantSlug
    ? `${window.location.origin}/t/${tenantSlug}/wofbi/checkin`
    : "";
  const churchName = currentTenant?.name || "Church";
  const label = `${churchName} — Bible School Attendance`;
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      a.download = `${tenantSlug || "church"}-bible-school-attendance-qr.png`;
      ...
      <DialogContent className="max-w-md w-[calc(100vw-1rem)] sm:w-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          <p className="text-xs text-muted-foreground text-center">
            One code for every session. When a session is open, students go straight to check-in.
            When multiple are open, they pick from a list. When none are open, they see a friendly notice.
          </p>
