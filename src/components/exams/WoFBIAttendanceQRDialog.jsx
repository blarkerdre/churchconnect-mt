...
export default function WoFBIAttendanceQRDialog({ open, onOpenChange, session, onClosed }) {
  const { currentTenant } = useTenant();
  const qrRef = useRef();
  const [count, setCount] = useState(0);
  const [closing, setClosing] = useState(false);

  const url = session ? `${window.location.origin}/wofbi/checkin/${session.qr_token}` : "";
  const churchName = currentTenant?.name || "Church";
  const label = `${churchName} — ${session?.title || "Bible School Attendance"}`;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`wofbi-att-${session.id}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "wofbi_attendance_records", filter: `session_id=eq.${session.id}` },
        () => load()
      )
      .subscribe();
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      a.download = `${session?.title?.replace(/\s+/g, "-") || "attendance"}-qr.png`;
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
            ...
            <span><strong className="text-foreground">{count}</strong> student{count === 1 ? "" : "s"} checked in</span>
          </div>
          ...
          <p className="text-[11px] text-muted-foreground text-center">
            Students scan this code on their phone to check in. They must be signed in and enrolled on the course.
          </p>
        </div>
      </DialogContent>
    </Dialog>
