...
export default function UnitTaskFormDialog({ open, onOpenChange, unitOptions = [], defaultUnit = "", task = null, onSaved }) {
  ...
  }, [open, defaultUnit, unitOptions, isEdit, task]);

  const { data: members = [], isLoading: membersLoading } = useQuery({
    queryKey: ["unit-members", tenantId, form.unit_name],
    enabled: !isEdit && !!tenantId && !!form.unit_name && open,
    queryFn: async () => {
  ...
  const submit = async () => {
    ...
    try {
      if (isEdit) {
        ...
        try {
          logAudit("unit_task.updated", "unit_task", task.id, {
            unit_name: form.unit_name, title: patch.title,
          }, tenantId);
        } catch { /* noop */ }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <TenantDialogHeader>{isEdit ? "Edit Unit Task" : "New Unit Task"}</TenantDialogHeader>
        <div className="space-y-4 py-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => { update("unit_name", v); setSelected(new Set()); }}
                disabled={isEdit}
              >
          ...
          {isEdit && (
            <p className="text-xs text-muted-foreground">
              To add or remove assignees, use the Assignees section in the task details.
            </p>
          )}
        </div>
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving}>
            {saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} {isEdit ? "Save Changes" : "Create Task"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
