...
export default function TeenAttendanceQRDialog({ open, onOpenChange, session, onClosed }) {
  const { currentTenant, tenantSlug } = useTenant();
  const qrRef = useRef();
  const [count, setCount] = useState(0);
  const [closing, setClosing] = useState(false);

  const path = tenantSlug ? `/t/${tenantSlug}/teens/checkin/${session?.qr_token}` : `/teens/checkin/${session?.qr_token}`;
  const url = session ? `${window.location.origin}${path}` : "";
  const churchName = currentTenant?.name || "Church";
  const label = `${churchName} — ${session?.title || "Teens Attendance"}`;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`teen-att-${session.id}`)
      .on("postgres_changes",
        { event: "*", schema: "public", table: "teen_attendance_records", filter: `session_id=eq.${session.id}` },
        () => load())
      .subscribe();
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      a.download = `${session?.title?.replace(/\s+/g, "-") || "teens-attendance"}-qr.png`;
  ...
  const handleClose = async () => {
    if (!session) return;
    setClosing(true);
    const { error } = await supabase.from("teen_attendance_sessions").update({ status: "closed" }).eq("id", session.id).eq("tenant_id", currentTenant?.id);
    setClosing(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <TenantDialogHeader>
          <QrCode className="h-5 w-5" /> Teens Check-in
        </TenantDialogHeader>
        <div className="space-y-4 py-2">
          ...
          <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
            ...
            <span><strong className="text-foreground">{count}</strong> teen{count === 1 ? "" : "s"} checked in</span>
          </div>
          ...
          <p className="text-[11px] text-muted-foreground text-center">
            Teens scan to check in. Parents sign in, or a worker signs the teen in manually.
          </p>
        </div>
      </DialogContent>
    </Dialog>
