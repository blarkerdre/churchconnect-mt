...
export default function WoFBICheckinLanding() {
  const { tenantSlug } = useParams();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [sessions, setSessions] = useState([]);

  const basePath = tenantSlug ? `/t/${tenantSlug}/wofbi/checkin` : `/wofbi/checkin`;

  useEffect(() => {
    let active = true;
    (async () => {
      setLoading(true);
      const { data } = await supabase.rpc("list_open_wofbi_sessions", { _tenant_slug: tenantSlug });
      if (!active) return;
      const list = data || [];
      setSessions(list);
      setLoading(false);
      if (list.length === 1) {
        navigate(`${basePath}/${list[0].qr_token}`, { replace: true });
      }
    })();
    return () => { active = false; };
  }, [tenantSlug, navigate, basePath]);
        ...
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="space-y-3 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              There's no open Bible School attendance session right now. Please wait for a leader
        ...
        <CardContent className="space-y-2">
          ...
          {sessions.map((s) => (
            <Button
              ...
              onClick={() => navigate(`${basePath}/${s.qr_token}`)}
            >
