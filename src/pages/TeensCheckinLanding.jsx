...
export default function TeensCheckinLanding() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const { data } = await supabase.rpc("list_open_teen_sessions", { _tenant_slug: tenantSlug });
      if (!active) return;
      const list = data || [];
      setSessions(list);
      setLoading(false);
      if (list.length === 1) {
        navigate(`/t/${tenantSlug}/teens/checkin/${list[0].qr_token}`, { replace: true });
      }
    })();
    return () => { active = false; };
  }, [tenantSlug, navigate]);
        ...
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="space-y-3 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              There's no open Teens Church attendance session right now. Please wait for a leader
        ...
        <CardContent className="space-y-2">
          ...
          {sessions.map((s) => (
            <Button
              ...
              onClick={() => navigate(`/t/${tenantSlug}/teens/checkin/${s.qr_token}`)}
            >
