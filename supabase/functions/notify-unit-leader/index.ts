...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { leader_user_id, member_name, unit_name, tenant_id } = await req.json();
...
<body style="margin:0;padding:0;background-color:#f4f5f7;font-family:Arial,Helvetica,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f5f7;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background-color:#ffffff;border-radius:8px;overflow:hidden;">
        ...
        <tr><td style="padding:32px;">
          ...
          <h2 style="margin:0 0 16px;color:#1a2d4d;font-size:18px;">New Member Joined Your Unit</h2>
          <div style="background-color:#f0f4f8;border-radius:8px;padding:16px;margin:0 0 24px;">
            ...
            <p style="margin:0;color:#555;font-size:14px;"><strong>Unit:</strong> ${escHtml(unit_name || "Unknown")}</p>
          </div>
          ...
          <p style="margin:0;color:#999;font-size:12px;text-align:center;">This is an automated notification from Unit Management.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    if (recipientPhone && smsEnabled) {
      ...
      if (LOVABLE_API_KEY && TWILIO_API_KEY && TWILIO_FROM) {
        ...
        if (/^\+[1-9]\d{6,14}$/.test(cleaned)) {
          const smsBody = `Hi ${recipientName}, ${member_name} has joined your unit: ${unit_name}. Please check the Church Management System. - ${churchShortName}`;

          try {
            const webhookUrl = `${supabaseUrl}/functions/v1/twilio-webhook`;
            const __quota = await checkSmsQuota(supabase, tenant_id, "sms", 1);
            if (!__quota.allowed) {
              console.warn("[notify-unit-leader] SMS quota exceeded for tenant", tenant_id, "— skipping send");
            } else {
            ...
            await supabase.from("sms_log").insert({
              ...
              message: smsBody,
              sms_type: "unit-leader-notification",
              reference_id: unit_name,
            ...
            if (response.ok) {
              console.log("Unit leader SMS sent to", cleaned);
            } else {
              console.error("Unit leader SMS failed:", data);
            }
          } catch (err) {
            console.error("Unit leader SMS error:", err);
          }
        }
      }
    }
    ...
    console.error("notify-unit-leader error:", error);
