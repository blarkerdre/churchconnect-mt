...
<p style="margin:0;color:#555;font-size:15px;">Please log in to acknowledge and complete this task.</p>
          ...
          try {
            await sendRawManagedEmail({
              ...
              html,
              text: `New task assigned in ${task.unit_name}: ${task.title}`,
              label: "unit-task-assignment",
              idempotencyKey: messageId,
              tenantId: task.tenant_id,
              messageId,
              fromName: churchShort,
            });
          } catch (sendError) {
            console.error("Failed to send unit-task email:", sendError);
          }
        }
