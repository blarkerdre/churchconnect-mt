...
BEGIN
  ...
  THEN
    ...
    -- Check if phone is valid E.164 (after basic normalization)
    _phone_valid := false;
    IF NEW.phone IS NOT NULL AND length(trim(NEW.phone)) > 5 THEN
      DECLARE
        _cleaned text;
      BEGIN
        _cleaned := regexp_replace(trim(NEW.phone), '[\s\-\(\)\.]', '', 'g');
        IF _cleaned ~ '^0[1-9]\d{9,10}$' THEN
          _cleaned := '+44' || substring(_cleaned from 2);
        END IF;
        IF left(_cleaned, 1) != '+' THEN
          _cleaned := '+' || _cleaned;
        END IF;
        IF _cleaned ~ '^\+[1-9]\d{6,14}$' THEN
          _phone_valid := true;
        END IF;
      END;
    END IF;

    -- Auto-schedule messages from templates (skip SMS if phone is invalid)
    FOR _tmpl IN
    ...
    LOOP
      -- Skip SMS templates when phone is invalid
      IF _tmpl.channel = 'sms' AND NOT _phone_valid THEN
        CONTINUE;
      END IF;
