...
CREATE OR REPLACE FUNCTION public.register_walkin_family(_tenant_id uuid, _parent jsonb, _children jsonb, _consent jsonb DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_c_given BOOLEAN := COALESCE((_consent->>'given')::boolean, false);
  v_c_photos BOOLEAN := COALESCE((_consent->>'photos')::boolean, false);
  v_c_pastoral BOOLEAN := COALESCE((_consent->>'pastoral')::boolean, true);
  v_c_emergency BOOLEAN := COALESCE((_consent->>'emergency')::boolean, false);
  v_c_notes TEXT := NULLIF(trim(_consent->>'notes'), '');
BEGIN
  ...
  END IF;
  IF NOT v_c_given THEN
    RAISE EXCEPTION 'Parental consent is required to register children';
  END IF;
  ...
  ELSE
    INSERT INTO public.members (
      ...
      _tenant_id, v_parent_first, v_parent_last, v_parent_phone, v_parent_email,
      'Visitor'::membership_status, 'children_church_walkin', v_parent_notes
  ...
  FOR v_child IN SELECT * FROM jsonb_array_elements(_children) LOOP
    INSERT INTO public.children (
      ...
      parental_consent_given, parental_consent_at, parental_consent_by,
      consent_photos, consent_pastoral_contact, consent_medical_emergency, consent_notes
    ) VALUES (
      ...
      true, now(), v_member_id,
      v_c_photos, v_c_pastoral, v_c_emergency, v_c_notes
    ...
    INSERT INTO public.consent_events (tenant_id, member_id, consent_type, granted, source)
    VALUES
      (_tenant_id, v_member_id, 'child_parental_master', true, 'children_church_walkin'),
      (_tenant_id, v_member_id, 'child_photos', v_c_photos, 'children_church_walkin'),
      (_tenant_id, v_member_id, 'child_pastoral_contact', v_c_pastoral, 'children_church_walkin'),
      (_tenant_id, v_member_id, 'child_medical_emergency', v_c_emergency, 'children_church_walkin');
...
GRANT EXECUTE ON FUNCTION public.register_walkin_family(UUID, JSONB, JSONB, JSONB) TO authenticated;
