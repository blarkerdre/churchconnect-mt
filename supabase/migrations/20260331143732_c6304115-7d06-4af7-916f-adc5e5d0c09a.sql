...
-- Drop ALL existing overloads by exact signature
...
-- 24-param version (no _workers_in_training)
DROP FUNCTION IF EXISTS public.update_own_member_profile(uuid,text,text,text,text,text,text,text,date,text,text,text,text,text,text,text,boolean,boolean,boolean,uuid,boolean,boolean,boolean,boolean);

-- 25-param version with text types
...
-- 34-param version with enum types (gender_type, membership_status)
...
-- 34-param version with text types
...
-- 35-param version with text types + _worshipped_at_other_wci
...
-- Now create the single canonical version with 35 params, all text for enum fields
...
BEGIN
  ...
  UPDATE public.members SET
    ...
    gender = CASE
      ...
      THEN _gender::public.gender_type
      ELSE gender
    END,
    ...
    membership_status = CASE
      ...
      THEN _membership_status::public.membership_status
    ...
    wsf_centre_id = CASE
      WHEN _winners_satellite = false THEN NULL
      ELSE COALESCE(_wsf_centre_id, wsf_centre_id)
    END,
