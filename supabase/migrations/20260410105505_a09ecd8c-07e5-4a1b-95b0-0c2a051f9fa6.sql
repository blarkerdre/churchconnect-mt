...
-- Prevent duplicate member records per user per tenant
CREATE UNIQUE INDEX idx_members_user_tenant_unique
  ON members (user_id, tenant_id)
  WHERE user_id IS NOT NULL;

-- Replace upsert_own_member_profile with email-fallback logic
...
BEGIN
  ...
  -- 2. Fallback: find unlinked member by email in the same tenant
  IF _member_id IS NULL THEN
    ...
    IF _user_email IS NOT NULL THEN
      ...
      -- Link the found member to this user
